...
| Bestellingen `/admin/bestellingen` | Leest bestellingen en toont statusbedragen/tellingen; orderdetailgegevens staan ook op klantdetail. | Alleen lezen, maximaal 100 orders in deze pagina; geen apart orderdetail/tijdlijn, refund, levering retry of export. Wise-checkout maakt na klantlogin een `pending_payment`-order via RPC en stuurt klant naar een vooraf aangemaakte Wise-link. Er is geen betaalbevestigingshandler aangetroffen: `/api/payment-status` geeft 503 en de Mollie-webhook 410. Wise-velden voor claim/verification bestaan in de migratie maar er is geen codeflow die ze gebruikt. |
...
| Instellingen `/admin/instellingen` | Leest integration-statusrijen en maximaal 12 bestaande werkitems; werkitemstatus kan worden aangepast en geaudit. | Status komt uit `integration_status`, niet uit een aangetroffen live check. Geen integratieconfiguratie-UI. “Actiecentrum” kan taak sluiten/starten, maar create/assign/triageactie is niet zichtbaar. |
...
- **Checkout bestaat als overdracht naar Wise, niet als betaalbevestiging.** Een DB-gedreven `/aanbod/[slug]`-pagina toont productprijs en QR-link; `POST /api/checkout` vereist een klantaccount, maakt/hergebruikt via `create_wise_checkout_order` één openstaande Wise-order per klant/product en redirect naar de opgeslagen Wise-link. De migratie beschrijft een vooraf in Wise Business aangemaakte vaste-betaallink. In de repository is geen verified callback, webhook of handmatige verificatie-API voor deze velden gevonden. De orderstatus gaat dus niet aantoonbaar naar betaald/geleverd via de aanwezige checkoutflow. De publieke `/profielen`- en `/modules`-catalogi blijven afzonderlijke statische content.
...
Deze nulmeting is afgeleid van lokale code en SQL-migraties in de repository op 24 september 2026. Ze controleert geen live website, Supabase-project, Auth-providerinstellingen, `.env`-waarden, productiegegevens, provideraccount, deploymentlogs of uitgevoerde back-ups. Een codepad bewijst dat een implementatie bestaat; het bewijst op zichzelf niet dat die in productie is ingeschakeld of correct geconfigureerd. Oudere documenten kunnen achterlopen; de verificatiestatus hierboven en actuele code zijn leidend totdat de andere docs zijn bijgewerkt.
