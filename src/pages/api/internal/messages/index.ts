...
const basePath = (value: FormDataEntryValue | null) => value === 'practice' ? '/praktijk/berichten' : '/admin/berichten?view=team';
...
export const POST: APIRoute = async ({ request, locals, url }) => {
  ...
  try {
    ...
    return Response.redirect(new URL(`${base}${separator}thread=${encodeURIComponent(threadId)}&created=1`, url), 303);
  } catch (error) {
    ...
    return Response.redirect(new URL(`${base}${base.includes('?') ? '&' : '?'}error=${encodeURIComponent(message)}`, url), 303);
  }
};
