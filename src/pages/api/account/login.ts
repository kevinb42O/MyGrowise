...
export const POST: APIRoute = async ({ request, cookies }) => {
  ...
  attempts.delete(key); cookies.set(SUPABASE_SESSION_COOKIE, authenticated.accessToken, supabaseSessionCookieOptions()); const next = sanitizeAppRedirect(form.get('next')); return Response.redirect(new URL(next || '/account', request.url), 303);
};
