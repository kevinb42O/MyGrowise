...
export const POST: APIRoute = async ({ request, cookies }) => {
  ...
  const { error } = await getSupabaseAdmin().from('analytics_events').insert({ event_name: event, anonymous_id: anonymousId, path, route_key: route || null, consented: true, environment: import.meta.env.PROD ? 'production' : 'preview' }); if (error) return new Response(null, { status: 500 });
