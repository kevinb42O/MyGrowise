...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  try {
    ...
    const event = await createPracticeAgendaEvent({ title: String(body.title || ''), startsAt: String(body.startsAt || ''), endsAt: String(body.endsAt || ''), kind: body.kind, location: String(body.location || '') }, locals.adminUser?.email || 'unknown');
...
export const PATCH: APIRoute = async ({ request, locals, url }) => {
  ...
  try {
    const body = await request.json();
    const event = await updatePracticeAgendaEvent(id, { title: String(body.title || ''), startsAt: String(body.startsAt || ''), endsAt: String(body.endsAt || ''), kind: body.kind, location: String(body.location || '') }, locals.adminUser?.email || 'unknown');
    return json({ event });
  } catch (error) { return json({ error: error instanceof Error && error.message === 'invalid_event' ? 'Controleer titel, type en tijdstip.' : 'Opslaan is niet gelukt.' }, 400); }
};
