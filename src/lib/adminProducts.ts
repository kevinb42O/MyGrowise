...
export type ProductFormFlash = { id?: string; error: string; fields: { title: string; slug: string; type: string; status: string; price: string; summary: string } };
...
export const encodeProductFormFlash = (form: FormData, error: string, id?: string) => Buffer.from(JSON.stringify({
  id, error, fields: {
    ...
    price: String(form.get('price') || '').slice(0, 10), summary: String(form.get('summary') || '').slice(0, 320),
  },
...
const rowToProduct = (row: Record<string, unknown>): AdminProduct => ({
  ...
  priceCents: row.price_cents === null ? null : Number(row.price_cents), currency: 'EUR', summary: String(row.summary),
...
const columns = 'id,title,slug,type,status,summary,price_cents,currency,created_at,updated_at,updated_by';
...
export const validateProductForm = (form: FormData): ProductValidation => {
  const title = String(form.get('title') || '').trim(); const slug = slugifyProduct(String(form.get('slug') || title)); const type = String(form.get('type') || '') as ProductType; const status = String(form.get('status') || '') as ProductStatus; const summary = String(form.get('summary') || '').trim(); const rawPrice = String(form.get('price') || '').trim().replace(',', '.'); const errors: Record<string, string> = {};
  ...
  return Object.keys(errors).length ? { errors } : { errors, value: { title, slug, type, status, summary, priceCents } };
};

const auditProduct = async (action: ProductAuditEvent['action'], product: AdminProduct, actor: string) => { const { error } = await getSupabaseAdmin().from('security_audit_log').insert({ action, object_type: 'product', object_id: product.id, metadata: { actor, after: product } }); databaseFailure(error); };
export const createProduct = async (input: ProductInput, actor: string) => { const { data, error } = await getSupabaseAdmin().from('products').insert({ title: input.title, slug: input.slug, type: input.type, status: input.status, summary: input.summary, price_cents: input.priceCents, currency: 'EUR' }).select(columns).single(); databaseFailure(error); const product = rowToProduct(data as Record<string, unknown>); await auditProduct('product.created', product, actor); return product; };
export const updateProduct = async (id: string, input: ProductInput, actor: string) => { const { data, error } = await getSupabaseAdmin().from('products').update({ title: input.title, slug: input.slug, type: input.type, status: input.status, summary: input.summary, price_cents: input.priceCents }).eq('id', id).select(columns).maybeSingle(); databaseFailure(error); if (!data) throw new Error('not_found'); const product = rowToProduct(data as Record<string, unknown>); await auditProduct('product.updated', product, actor); return product; };
